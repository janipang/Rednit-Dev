...
namespace RednitDev.Components{
    public class PostViewComponent: ViewComponent
    {
        ...
        public static string GetViewComponent(Post post){
            string tags = "";
            string images = "";
            foreach(var tag in post.Detail.Tag){
...
<div class="post-container">
    <div class="post-top-section">
        <div class="post-top-left-section">
            ...
            <div class="side-user-image">
                <button class="post-username">{post.Author.Username}</button>
                <div class="other-detail">
                    <svg width="15" height="15" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8.5 16C12.6421 16 16 12.6421 16 8.5C16 4.35786 12.6421 1 8.5 1C4.35786 1 1 4.35786 1 8.5C1 12.6421 4.35786 16 8.5 16Z" stroke="#AAAAAA" stroke-width="1.5" stroke-linecap="round"/>
                        <path d="M9 4V8.5L12 10" stroke="#AAAAAA" stroke-width="1.5" stroke-linecap="round"/>
                    </svg>
                    <span class="ramaining-day">
                        {post.DayLeft} Day{(post.DayLeft > 1 ? "s" : "")} left 
                    </span>
                    {(post.Requesting ? 
        ...
        <div class="post-top-right-section">
            {tags}
            <button onClick="onFavButtonClick({post.Id})">
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"
                    style="margin-left: 5px">
                    <path class="favIcon" name="{post.Id}"
    ...
    <div class="post-detail-section">
            <p class="post-title">{post.Detail.Header}</p>
            <p class="post-intro">{post.Detail.Intro}</p>
    </div>
    <div class="post-bottom-section">
        ...
        <div>
            ...
            <span>{post.EventDate.Start + (post.EventDate.DateType != "one" ? " - " + post.EventDate.End : "")}</span>
        </div>
        <div>
            ...
            <span class="post-member-section">
                <div>{post.Joined.Count}/{post.MemberMax}</div>
        ...
        <button class="show-detail-button" onclick="onMoreDetailButtonClick({post.Id})">More Detail</button>
    </div>
</div>

{"""<script>function onMoreDetailButtonClick(postID){var url = '@Url.Action("ViewPost", "Discover")' + '?id=' + encodeURIComponent(postID + "");window.location.href = url;}</script>"""}
{"""<script>function onFavButtonClick(postID){ var favIcon = document.getElementsByClassName("favIcon"); var thisIcon = document.getElementsByName(`${postID}`)[0]; console.log("fav clicked with Id = " + postID); $.ajax({ url: '/Discover/AddToFav', type: 'GET', data: { postId: postID}, success: function(isLiked) { console.log("received isLiked" + isLiked); if(isLiked){ thisIcon.setAttribute("stroke","#FFFFFF"); thisIcon.setAttribute("fill","#FE3C72"); console.log("liked post"); } else{ thisIcon.setAttribute("stroke","#FE3C72"); thisIcon.setAttribute("fill","#FFFFFF"); } }, error: function(xhr, status, error) { console.error('Error:', status, error); } }); }</script>"""}
""";
        }
    }
}
